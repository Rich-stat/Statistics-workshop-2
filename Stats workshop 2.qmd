...
<br> **1. Download the Excel file** [1-Factor_ANOVA_cholesterol.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQBqe4vRCqtaQZkUCsQXz_YFAQY7HNFEAVfAsx9N1GevNyU?e=h6fgHF) from Canvas to your Downloads folder and make it available to RStudio.
...
#| echo: TRUE
#| eval: false
describeBy(Cholesterol ~ Group, data = anova_1_factor, quant = c(.25, .75)) #<1>
```
...
#| echo: false
#| eval: true
describeBy(Cholesterol ~ Group, data = anova_1_factor, quant = c(.25, .75))
```
:::
...
**1. Download and prepare the data**<br> Download the excel file [2-factor_ANOVA_cholesterol.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQBwHoT-hmy4RoYHZWIcrl-CAUCnuYRgSQD2SshYSLXKsIE?e=aqUyvs) from Canvas to your Downloads folder and make it available to RStudio.
...
#| messages: FALSE
#| eval: false
describeBy(Cholesterol ~ Group + Gender, data = aov_2_factor, quant = c(.25, .75)) #<1>
```
...
#| messages: FALSE
#| eval: true
describeBy(Cholesterol ~ Group + Gender, data = aov_2_factor, quant = c(.25, .75))
```
:::
...
<br><br> `5.2` **Multiple comparison test:**<br> Compares every group with every other group.<br><br> `5.2a` If variances in the 2 factors are equal (`4.2` above), use the **Tukey HSD** multiple comparison test.<br>
...
::: {.callout-note collapse="true" title="Click to see the Tukey HSD multiple comparison output" icon="false"}
```{r}
#| label: mc2fact1
#| echo: false
#| eval: true
...
<br> `5.2b` If homogeneity of variance is NOT true (`4.2` above), use the **Games-Howell** multiple comparison test:<br>

- First, create a column in the dataframe for the interaction term.<br>
- Then, one has to do 3 separate Games-Howell tests: one for each of the factors and one for the interaction.
...
#| eval: false
aov_2_factor <- aov_2_factor |>
  mutate(inter = interaction(Group, Gender))  #<1>

aov_2_factor |>
  games_howell_test(Cholesterol ~ inter)  #<2>
aov_2_factor |>
  games_howell_test(Cholesterol ~ Group) #<3>
aov_2_factor |>
  games_howell_test(Cholesterol ~ Gender) #<4>
```

1.  Sends the dataframe to the mutate function and creates a new column called 'inter' in which the interaction term is calculated. Stores the revised dataframe with the same name.
2.  Does a multiple comparison for the interaction of Group & Gender
3.  Does a multiple comparison for the Group factor
4.  Does a multiple comparison for the Gender factor

::: {.callout-note collapse="true" title="Click to see the Games-Howell multiple comparison test output for a 2-factor ANOVA" icon="false"}
```{r}
#| label: GH_2-factor2
#| echo: false
#| eval: true
aov_2_factor <- aov_2_factor |>
  mutate(inter = interaction(Group, Gender))

aov_2_factor |>
  games_howell_test(Cholesterol ~ inter)
aov_2_factor |>
  games_howell_test(Cholesterol ~ Group)
aov_2_factor |>
  games_howell_test(Cholesterol ~ Gender)
```
:::
...
<br>**1. Download the file** [ANOVA_RM_cholesterol.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQCQomrPXIAsRIlzuCV4FE7BAWXxkSgcviim1IQvcxgS7ts?e=S6hLiN) from Canvas to your Downloads folder and make it available to RStudio.
...
#| echo: TRUE
#| eval: false
   describeBy(Cholesterol ~ Time, data = chol_RM, quant = c(.25, .75))   #<1>
```
...
#| echo: false
#| eval: true
   describeBy(Cholesterol ~ Time, data = chol_RM, quant = c(.25, .75))
```
:::
...
<br>`1.` **Download the data**<br> Download the excel file [ANCOVA_cholesterol.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQDKNaugiVWnTriFRHVADTdaAZ5r9nDMZhwiKYiPQ3vM_eI?e=HQbrnK) from Canvas to your Downloads folder and make it available to RStudio.
...
#| echo: TRUE
#| eval: false
describeBy(Cholesterol ~ Group, data = ancova_chol, quant = c(.25, .75))
```
